...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        for e in range(epochs):
            shuffle_sdp_id_padding, shuffle_labels = randomize(sdp_id_padding, labels)
            each_fold_size = sdp_id_padding.shape[0] // k_fold
            ...
            for k in range(k_fold):
                valid_dataset = shuffle_sdp_id_padding[k*each_fold_size:(k+1)*each_fold_size, :]
                valid_labels = shuffle_labels[k*each_fold_size:(k+1)*each_fold_size, :]
                train_dataset = np.delete(shuffle_sdp_id_padding, range(k*each_fold_size, (k+1)*each_fold_size), axis=0)
                train_labels = np.delete(shuffle_labels, range(k*each_fold_size, (k+1)*each_fold_size), axis=0)
